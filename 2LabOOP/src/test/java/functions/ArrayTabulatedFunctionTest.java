...
public class ArrayTabulatedFunctionTest {
    ...
    void testRemoveFromSizeOne() {
        ...
        assertThrows(IndexOutOfBoundsException.class, () -> f.getX(0), "getX(0) на пустой таблице — исключение GOOD");
        ...
        assertThrows(IndexOutOfBoundsException.class, () -> f.extrapolateLeft(15.0), "extrapolateLeft на пустой таблице — исключение GOOD");
        assertThrows(IndexOutOfBoundsException.class, () -> f.extrapolateRight(15.0), "extrapolateRight на пустой таблице — исключение GOOD");
    ...
    void testAllMethodsThrowAfterLastPointRemoved() {
        ...
        assertThrows(IndexOutOfBoundsException.class, () -> f.extrapolateLeft(0.5), "extrapolateLeft — исключение GOOD");
        assertThrows(IndexOutOfBoundsException.class, () -> f.extrapolateRight(0.5), "extrapolateRight — исключение GOOD");
    ...
    @Test
    @DisplayName("Автоматическое увеличение емкости массивов")
    public void testCapacityIncrease() {
        double[] xValues = {1.0};
        double[] yValues = {1.0};
        ArrayTabulatedFunction function = new ArrayTabulatedFunction(xValues, yValues);

        int initialCapacity = function.getxVal().length;

        for (int i = 2; i <= 20; i++) {
            function.insert(i, i * i);
        }

        assertEquals(20, function.getCount(), "Должно быть 20 точек после вставок");
        assertTrue(function.getxVal().length > initialCapacity, "Емкость массивов должна увеличиться");

        // Проверяем корректность данных после расширения
        for (int i = 0; i < 20; i++) {
            double expectedX = i + 1;
            double expectedY = expectedX * expectedX;
            assertEquals(expectedX, function.getX(i), 1e-10,
                    "X значение точки " + i + " должно быть корректным");
            assertEquals(expectedY, function.getY(i), 1e-10,
                    "Y значение точки " + i + " должно быть корректным");
        }
    }
